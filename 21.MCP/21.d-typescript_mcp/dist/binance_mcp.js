...
async ({ symbol }) => {
    ...
    if (!response.ok) {
        ...
        fs_1.default.appendFileSync(ACTIVITY_LOG_FILE, `Error obteniendo el precio para ${resolvedSymbol}: ${response.status} ${errorText}\n`);
        throw new Error(`Error obteniendo el precio para ${resolvedSymbol}: ${response.status} ${errorText}`);
    }
    ...
    fs_1.default.appendFileSync(ACTIVITY_LOG_FILE, `Se obtuvo el precio para ${resolvedSymbol} con éxito. El precio actual es ${price}. La hora actual es ${new Date().toISOString()}\n`);
...
server.tool("get_price_price_change", { symbol: zod_1.z.string() }, async ({ symbol }) => {
    ...
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error(`Error obteniendo el cambio de precio para ${resolvedSymbol}: ${response.status}`);
    }
    const data = (await response.json());
...
async function startServer() {
    ...
    console.error("Recursos: file://activity.log, resource://crypto_price/{symbol}");
}
